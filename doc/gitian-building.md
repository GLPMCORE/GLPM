...
*Setup instructions for a gitian build of GLPM using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the GLPM
...
Clone the git repositories for GLPM and gitian and then checkout the GLPM version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the GLPM repository
...
To build GLPM (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the GLPM repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/GLPM/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/eastcoastcrypto/GLPM
...
For example:
```bash
URL=https://github.com/eastcoastcrypto/GLPM.git
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/GLPM-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/GLPM-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/GLPM-build.assert
```
